...
Ext.define('Shopware.apps.sKUZOOffer.view.list.Offer', {
     ...
     * @string
     */
    alias:'widget.offer-list',

    /**
     * Set css class
     * @string
     */
    cls:Ext.baseCSSPrefix + 'offer-grid',
    ...
    snippets:{
        columns: {
            ...
            deleteOffer: '{s name=column/delete_offer}Delete offer{/s}',
        ...
        toolbar: {
            ...
            action: '{s name=toolbar/action}Actions{/s}',
            newOffer: '{s name=toolbar/newOffer}New Offer{/s}',
           removeAll: '{s name=toolbar/removeAll}Delete all selected{/s}'
        },
        paging: {
            ...
            rowEditor = Ext.create('Ext.grid.plugin.RowEditing', {
                ...
                listeners: {
                    ...
                    edit: function(editor, e) {
                        me.fireEvent('saveOffer', editor, e, me.offerStore)
                    }
                }
            });
    ...
    getColumns:function () {
        ...
        var columns = [
            ...
            {
                header: me.snippets.columns.discountAmount,
            ...
            {
                header: me.snippets.columns.shipping,
                dataIndex: 'invoiceShipping',
                flex:1,
                renderer:  Ext.util.Format.numberRenderer('0.00')
    ...
    createDeleteOfferColumn: function() {
        var me = this;
        return {
            iconCls:'sprite-minus-circle-frame',
            action:'deleteOffer',
            tooltip:me.snippets.columns.deleteOffer,

            handler:function (view, rowIndex, colIndex, item) {
                var store = view.getStore(),
                    record = store.getAt(rowIndex);

                me.fireEvent('deleteOffer', record);
            }
        };
    },
    ...
    getToolbar:function () {
        ...
        me.createOfferButton = Ext.create('Ext.button.Button', {
            iconCls:'sprite-plus-circle-frame',
            text:me.snippets.toolbar.newOffer,
            handler: function(grid) {
                me.fireEvent('createOffer', me);
            }
        });
        me.deletePositionsButton = Ext.create('Ext.button.Button', {
            iconCls:'sprite-minus-circle-frame',
            text:me.snippets.toolbar.removeAll,
            disabled:true,
            action:'deleteOffer',
            handler: function() {

                me.fireEvent('deleteMultiplePositions', {
                ...
                '->',
                {
                    ...
                    listeners: {
                        change: function(field, value) {
                            me.fireEvent('searchOffers', value);
                        }
                    }
                },
