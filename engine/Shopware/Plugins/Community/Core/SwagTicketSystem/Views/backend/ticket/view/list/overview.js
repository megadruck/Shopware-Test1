...
Ext.define('Shopware.apps.Ticket.view.list.Overview', {
    ...
    defaultColumnRenderer: function(value, meta, record) {
        return this.readRenderer(value, record);
    },

    readRenderer: function(value, record) {
        var isRead = record.get('isRead');
    ...
    createActionToolbar: function() {
        ...
        return Ext.create('Ext.toolbar.Toolbar', {
            ...
            /*{/if}*/
            {
                ...
                displayField: 'name',
                emptyText: '{s name=toolbar/combo_empty}Please select...{/s}',
                store: me.employeeComboStore.load(),
                listeners: {
                    scope: me,
                    change: function(field, newValue, oldValue) {
    ...
    getActionColumnItems: function() {
        ...
        actionColumnData.push( {
			...
			handler: function(view, rowIdx, colIdx, item, e, record) {
				if (record.get('userId') != 0) {
					...
					Shopware.Notification.createGrowlMessage('{s name=overview/columns/open_customer_error}Error!{/s}', '{s name=overview/columns/open_customer_errorText}The selected ticket was not created by a registered customer.{/s}');
				}
            }
        });
        /*{/if}*/
        ...
        actionColumnData.push( {
            ...
            tooltip: '{s name=overview/columns/print_ticket}Print ticket{/s}',
            handler: function(view, rowIdx, colIdx, item, e, record) {
                window.open("{url action='print'}?ticketId=" + record.get('id'));
            }
        });
        /*{/if}*/
        ...
        actionColumnData.push({
            ...
            handler: function(view, rowIdx, colIdx, item, e, record) {
                me.fireEvent('editTicket', view, record, record.get('albumId'), me.employeeStore, me.employeeId, rowIdx, colIdx, item, e);
            }
        });
        /*{/if}*/
    ...
    emailRenderer: function(value, meta, record) {
        ...
        if(!value) {
            return this.readRenderer('<a href="mailto:' + email + '">' + email + '<a/>', record);
        }
        return this.readRenderer('<a href="mailto:' + email + '">' + value + '<a/>', record);
    },
    ...
    statusRenderer: function(value, meta, record) {
        ...
        return this.readRenderer(status.get('description'), record);
    },
    ...
    employeeRenderer: function(value, meta, record) {
        ...
        return this.readRenderer(employee.get('name'), record);
    },
    ...
    gridColorRenderer: function(value, meta, record) {
        ...
        return '<div style="width:14px;height:14px;background-color: '+ color +';display:inline-block;vertical-align: middle"></div>&nbsp;#' + this.readRenderer(value, record);
    },

    getCurrentEmployeeId:function() {
        var me = this;
        Ext.Ajax.request({
            url: '{url controller=Ticket action=getCurrentEmployeeId}',
            success: function(datax) {
                var response = Ext.decode(datax.responseText);
                me.employeeId = response['userID'];
            }
        });
    }
});
