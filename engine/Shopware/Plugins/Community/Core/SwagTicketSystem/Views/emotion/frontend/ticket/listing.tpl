...
<div id="ticket" class="grid_16">
	...
	<div class="ticketoverview">
		...
		<div class="content">
			{foreach from=$ticketStore item=ticketItem}
				...
				<div class="row" style="background-color:{$column_color}">
					<div class="grid_3">{$ticketItem.receipt|date_format:"%d.%m.%Y %H:%M:%S"}</div>
					<div class="grid_3">#{$ticketItem.id}</div>
					<div class="grid_3" style="{if $ticketItem.statusColor != '0'}color:{$ticketItem.statusColor}{/if}">{$ticketItem.status}</div>
					<div class="grid_3" style="background-color:{$column_color};">
						<a href="{url controller='ticket' action='detail' tid=$ticketItem.uniqueId}" class="button-middle small">
